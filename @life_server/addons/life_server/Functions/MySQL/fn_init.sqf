...
if ( isNil {uiNamespace getVariable "extDB_SQL_CUSTOM_ID"}) then
{
	...
	_result = call compile ("extDB" callExtension format["9:ADD_DATABASE:%1", _database]);
	if (_result select 0 isEqualTo 0) exitWith {diag_log format ["extDB: Error Database: %1", _result]; false};
	...
	// extDB Load Protocol
	_result = call compile ("extDB" callExtension format["9:ADD_DATABASE_PROTOCOL:%1:%2:%3:%4", _database, _protocol, _extDB_SQL_CUSTOM_ID, _protocol_options]);
	if ((_result select 0) isEqualTo 0) exitWith {diag_log format ["extDB: Error Database Setup: %1", _result]; false};
	_result = call compile ("extDB" callExtension format["9:ADD_PROTOCOL:LOG:DEBUG:debug.log"]);
